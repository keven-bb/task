...
export class Tokens {
  ...
  public static set conn(value: DBPool) {
    this._conn = value
  }
  ...
  public static async getTokenByAddress(address: string): Promise<IToken> {
    ...
    let [id, , current] = result[0] as any
    return {id, address: add0x(address), current}
  }
  ...
  public static async addToken(address: string, current: number): Promise<IToken> {
    ...
    await this._conn.pool.query(`insert into ${Tokens._tableName} (address, current) values (?, ?);`, [
      address,
      current,
    ])
  ...
  public static async count(): Promise<number> {
    const result = await this._conn.pool.query(`select count('id') from ${Tokens._tableName}`)
